...
Ask (via `request_user_input` in Plan mode when available, or directly otherwise) only when the diff source is genuinely ambiguous — most commonly **several
...
summary, the instruction-file paths, and the finding schema from `references/state.md`.
...
positive, re-check any instruction-derived finding against the actual rule text, and **escalate to
...
prior plan. Enter Codex Plan mode, write the plan artifact, and present it for approval. This is the only channel for proposing edits.
...
- Tune thresholds/rosters in the reference files, or per-repo via a `.review-branch.md` (falling back
  to `AGENTS.md` or `CLAUDE.md`) in the target repo.
