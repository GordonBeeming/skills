...
Enter Codex Plan mode and use `request_user_input` for the approval gate when available.
...
Present the completed plan and wait for approval before mutating any pull request.
...
If the build command is not obvious from the repo, check for `AGENTS.md`, any applicable `CLAUDE.md`, `Makefile`, `package.json` scripts, or similar. Run whatever the repo's standard build command is.
