...
> **Reply-before-resolve invariant.** Every thread ends either replied-and-resolved or escalated. Re-fetch all threads each tick, including already-resolved threads that still lack our reply; zero unresolved is necessary but not sufficient. Never resolve first and plan to reply later.

This reference is part of the `pull-request` skill. It runs in one of two modes:

- **Direct mode** — the main session runs it itself (User asked for a one-off resolve round). Decision gates go straight to User using Plan mode and `request_user_input` when available, or a direct blocking question otherwise, exactly as written in each step.
- **Delegated mode** — a teammate runs it inside an autopilot loop. The teammate has no channel to User, so every decision gate becomes a `send_message` to the lead instead: send the same block content the step specifies (skip-plan template, scope-creep template, re-opened table, question-only table), then wait for the lead's reply before acting. The lead decides what the authority table in SKILL.md allows and forwards the rest to User. Never substitute your own judgment for a gate — if the step says gate, you gate, in both modes.
...
**This gate always ends at User, in both modes and both repo tiers.** In direct mode, surface the table below as a standalone blocking question before any other proposed work. In delegated mode, `send_message` the same table to the lead flagged `re-opened threads — User required`; the lead forwards it to User and nobody touches the threads meanwhile.
...
**Convention / standard-design deviation — always gate.** If the diff breaks an established convention, ADR, or prior decision, or a proposed fix would introduce that deviation, neither auto-fix nor auto-skip it. Name the convention, deviation, and cost; route an explicit "override the convention" choice to User (delegated mode: via the lead). Conform unless User chooses the override.
...
3. **For medium or large changes** → open the gate before taking any action. Direct mode: use Plan mode and `request_user_input` when available, or ask directly. Delegated mode: `send_message` the same content to the lead — on **personal-tier repos** (see SKILL.md) the lead approves or rejects the split itself, escalating to User only when genuinely uncertain; on **standard-tier repos** the lead forwards to User. The gate content must include:
...
1. **Write the skip-plan content** using this exact section structure (template — fill each section, do not skip any). Direct mode: make it the complete blocking approval artifact; do not bury it in a status update. Delegated mode: the same content is the `send_message` body.
...
2. Open the gate (direct: Plan mode plus `request_user_input` when available, otherwise a direct blocking question; delegated: `send_message` to lead) and wait for the decision. Never proceed on a skip without explicit approval from the gate.
3. On **skip approved**:
...
3. **Make the fix** following the project's coding standards (check `AGENTS.md` and any applicable `CLAUDE.md`).
...
- **Gates are mode-routed, never skipped.** Direct mode: Plan mode plus `request_user_input` when available, otherwise a direct question to User. Delegated mode: `send_message` to the lead, who applies the SKILL.md authority table. The gate content is identical either way.
...
- **Respect project conventions** — read `AGENTS.md` and any applicable `CLAUDE.md`, then follow established patterns.
