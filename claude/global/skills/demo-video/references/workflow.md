...
- **If there were off-camera steps (step 0/2), the interstitial is not optional** — render it AND include its
  `imageClip` in the `concat` list, positioned right before the recorded section it sets up. A rendered-but-
  not-concatenated interstitial is a common miss; the symptom is exactly "the behind-the-scenes page isn't in
  the video" (the QA pass in `qa-loops.md` §2 catches this against the actual output).
