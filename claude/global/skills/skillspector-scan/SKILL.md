...
If `target` is missing, ask for it with `AskUserQuestion` before running — never guess one.
...
- Key reference: `op://ai-secrets/Anthropic API key - DONT JUST USE/password`. If `op read`/`op run` fails (not signed in), tell the user to run `op signin` — don't fall back to another provider or an unauthenticated run silently.
