...
4. **Cap the dynamic add-ons at 4 per pass.** If more than 4 signals match, keep the 4 most clearly
   evidenced by the diff (the ones with the most touched lines matching the signal) and note in
   `progress.md` which were dropped and why — a diff rarely genuinely needs more than 4 extra angles
   beyond the five-expert core, and an uncapped panel adds fan-out cost without proportional signal.
5. Record the chosen roster in `progress.md` so the selection is observable and tunable.
