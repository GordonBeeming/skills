...
description: >
  ...
  `/pr-change-map`. Never trigger on a pasted PR URL, "what does this PR do", or "explain this PR";
  that intent belongs to review-pr. Not a quality review: no severity ranking, no approve/reject
  verdict, no GitHub comments, no code edits.
---
...
8. **Render and visually check it** (Playwright MCP visual QA loop, per the global rules).
...
- Deliver the map that was asked for. Don't expand into fix suggestions, follow-up tickets, or a
  refactor proposal.
